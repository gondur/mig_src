...
#define MSS_VERSION      "4.0e"
#define MSS_VERSION_DATE "10-May-98"
...
#define DLS_ENABLE_FILTERING       25   // No filtering by default
...
#define DLS_ENABLE_MMX_SUPPORT     27   // Enable MMX support if present
#define DEFAULT_DEMS               YES  // (may be changed at any time)
...
#define N_PREFS 31                        // # of preference types
...
typedef struct _MEMDUMP {
  void FAR* buffer[256]; // up to 16 MB
  U8 FAR* current;
  U32 size;
  U32 totalsize;
  U32 curbufnum;
...
U32 _cdecl AIL_mem_printf(HMEMDUMP m, char FAR* fmt, ...);

U32 AIL_mem_printc(HMEMDUMP m, char c);

U32 AIL_mem_prints(HMEMDUMP m, char FAR* s);

U32 AIL_mem_write(HMEMDUMP m, void FAR* s, U32 bytes);
...
U32  cdecl AIL_ferase(char* dest);
...
void AILCALL AIL_memmove(void* dest,const void* source,U32 size);
...
DXDEC S32 __cdecl (*AIL_sprintf) (char FAR *dest, char FAR *fmt, ...);
DXDEC S32 __cdecl (*AIL_fprintf) (U32 hand, char FAR *fmt, ...);
...
extern S32 AIL_ptr_dif(void FAR *p1, void FAR *p2);

extern void FAR *AIL_ptr_alloc_clone(void FAR *src);
...
extern S32 _cdecl AIL_sprintf            (char FAR *dest,
                                          char FAR *fmt, ...);
...
DXDEC void     AILCALL AIL_set_stream_user_data   (HSAMPLE S,
                                                     U32     index,
                                                     S32     value);

DXDEC S32      AILCALL AIL_stream_user_data       (HSAMPLE S,
                                                     U32     index);
